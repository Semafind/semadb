...
func randDistElems(dists ...float32) []DistSetElem {
	...
	for i, dist := range dists {
		rid := uuid.New()
		elems[i] = DistSetElem{distance: dist, ShardPoint: ShardPoint{Point: models.Point{Id: rid}}}
	}
	return elems
}
...
func TestDistSet_Add(t *testing.T) {
	...
	for i, elem := range ds.items {
		assert.Equal(t, elems[wantOrder[i]].Id, elem.Id)
	}
	ds.Sort()
	fmt.Println(ds)
...
func TestDistSet_Pop_Remove(t *testing.T) {
	...
	ds.Sort()
	assert.Equal(t, 3, ds.Len())
	assert.Equal(t, elems[2].Id, ds.Pop().Id)
	assert.False(t, ds.Contains(elems[2].Id))
	ds.Remove(elems[0].Id)
	assert.Equal(t, 1, ds.Len())
	assert.Equal(t, elems[1].Id, ds.Pop().Id)
